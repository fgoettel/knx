...
class Knxproj:
    ...
    def __init__(self, knxproj_path: Path) -> None:
        ...
        if project_id not in str(self.xml_project_path) or project_id not in str(
            ...
            msg = "Inconsistent project id found."
            raise RuntimeError(msg)
        self.project_prefix = project_id + "-" + self.project_file_name.split(".", maxsplit=1)[0]
    ...
    def _unzip(cls: type["Knxproj"], knxproj_path: Path) -> tuple[Path, Path]:
        ...
        with ZipFile(knxproj_path, "r") as zip_:
            ...
            project_path = Path(
                zip_.extract(
                    next(filter(lambda x: x.endswith(cls.project_file_name), xml_files)),
                    unzip_folder,
                ),
            )
            meta_path = Path(
                zip_.extract(
                    next(filter(lambda x: x.endswith(cls.meta_file_name), xml_files)),
                    unzip_folder,
                ),
            )
    ...
    def _extract_ets_version(self) -> str:
        ...
        return root.attrib["CreatedBy"].lower() + root.attrib["ToolVersion"].split(".")[1]
